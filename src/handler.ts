...
> = async (event) => {
  ...
  const result = await handle(`https://node.ytoo.site/api/v1/subtrans?url=${commonSubUrlBase64}&dest=surge`, event.multiValueQueryStringParameters, ytooResolver, sortMethod);
  return {
    statusCode: 200,
    headers: {"content-type": "text/plain"},
    body: result

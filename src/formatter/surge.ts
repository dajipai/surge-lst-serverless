    ...
    format(proxy: Proxy) : string {
        if (proxy instanceof ShadowsocksProxy) {
            ...
            if (proxy.obfs === undefined) {
                res += `ss, ${proxy.host}, ${proxy.port}, encrypt-method=${proxy.encryptionMethod}, password=${proxy.password}`;
            } else {
                res += `ss, ${proxy.host}, ${proxy.port}, encrypt-method=${proxy.encryptionMethod}, password=${proxy.password}, obfs=${proxy.obfs}, obfs-host=${proxy.obfsHost}`;
            }
            if (proxy.udpRelay) {
                res += ", udp-relay=true";
            }
